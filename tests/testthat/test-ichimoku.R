...
strat <- strat(cloud)
xtsobject <- xts::xts(sample_ohlc_data[, -1], order.by = sample_ohlc_data[, 1])
...
test_that("ichimoku object specification correct", {
  ...
  names <- c("open", "high", "low", "close", "cd", "tenkan", "kijun", "senkouA", "senkouB", "chikou", "cloudT", "cloudB")
...
test_that("ichimoku keep.data ok", {
  ...
  expect_identical(cloud, ichimoku(sample_ohlc_data[, -6L], ticker = "TKR", keep.data = TRUE))
...
test_that("ichimoku error handling ok", {
  ...
  expect_error(ichimoku(sample_ohlc_data[, -5]), regexp = "price data not found")
  data$time <- 1:256
