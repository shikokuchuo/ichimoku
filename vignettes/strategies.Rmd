...
To test a simple strategy with a rule condition of the form 'long (or short) while c1 > c2'.
...
Note: the rule condition is c1 > c2 even for short trades. Hence please take care for example when inverting 'long while close > tenkan' to 'short while close < tenkan', this should be formulated as 'short while tenkan > close'.
...
Strategies may be combined to form more complex strategies if desired. The function can be used on 2 ichimoku objects containing rule conditions 'c1 > c2' and 'c3 > c4' respectively, combining them into a new strategy with a rule condition of 'c1 > c2 & c3 > c4'.
...
Set the `level = 2` argument to `autostrat()` to test all strategies with a combination of up to 2 rule conditions.
