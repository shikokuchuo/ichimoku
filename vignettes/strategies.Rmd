---
title: "ichimoku: Strategies"
output:
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{ichimoku: Strategies}
...
2. Use `strat()` to test a simple strategy with a rule of the form 'long (or short) while c1 > c2'.
...
- **cond**: '1' if the rule condition is met at the close of the the time period (note: if the rule involves chikou span, the comparison will be of past periods)
...
The summary is meant to provide a quick overview of whether a strategy is effective or not.
...
- Winning trade % helps to characterise the consistency of the strategy or whether it relies on a small percentage of outsize winners.
...
The periods where the strategy results in a market position are now highlighted on the ichimoku cloud chart. To view the original chart without the strategy, simply pass the argument `strat = FALSE` when calling `plot()`.

Note: `iplot()` interactive charting currently does not implement this additional functionality for strategies.

5. Combining strategies using `stratcombine()`

Strategies may be combined to form more complex strategies if desired. The function can be used on 2 ichimoku objects containing rules c1 > c2 and c3 > c4 respectively, combining them into a new strategy with a rule of c1 > c2 & c3 > c4.
...
The strategy returns are saved within the ichimoku object in the columns 'slogret' (strategy log returns) and 'sret' (strategy discrete returns). The returns (benchmark) for all periods are saved as 'logret' (log returns) and 'ret' (discrete returns). As the ichimoku object inherits the 'xts' class, these columns may be fed directly into other econometrics or analytics packages such as 'PerformanceAnalytics', as per the example below.
...
- **level** [default '1'] '1' or '2' to also return combined strategies

```{r autostrat}
autostrat(cloud, n = 3)
```

The output of autostrat is a list of ichimoku objects. Each object can be accessed by its individual item number e.g. [[1]] for the first object.

The metadata is also saved as attributes to the list and can be accessed by the function `attributes()`:

- **summary**: the strategy summaries

- **logret**: the log returns of all combinations computed by autostrat

Note: the strategies returned may not be in perfect order of strategy returns. This is due to the fact that the implementation via `mlgrid()` simultaneously tests all strategies using the same time interval for comparability. However individual strategies are then run on the top 'n' strategies, and this uses all of the available data for those indicators, which may be more than that used during comparison.

Note: as SenkouA, SenkouB, cloudTop and cloudBase are used in conjuction with other indicators, it is possible to get a series of similar returns with cloudBase > c2, senkouB > c2 etc. Although these strategies may at times be equivalent or considered equivalent, this is not always the case and ichimoku will presents all results.

### autostrat() level 2

Set the `level = 2` argument to `autostrat()` to test all strategies with a combination of up to 2 indicator rules.
...
**Winning trades %**: Number of trades where the return is strictly greater than zero divided by the total number of trades.
