...
Produces a statistical summary of the latest numeric representation of the ichimoku cloud chart relative to historical values contained within the ichimoku object. This can be used to determine whether current trading falls within or outside of normal ranges.

```{r relative}
relative(cloud)[1:10, ]
```

The mean and standard deviation of each element is calculated, with the latest values shown under 'latest'.

The 'relative' or 'r value' is calculated as (latest - mean) / sd and represents a standardised (centred and scaled) measure of deviation.

Column 'p >= |r|' contains the empirical probability of the observed absolute 'r value' or greater.

Column 'E(|r|)|p' then contains the expected absolute 'r value', conditional upon being greater than equal to the observed absolute 'r value'. This provides an indication of how much further the measure may reach in more extreme cases.

The elements are ordered by their absolute 'r value', so that those with the highest relative deviation are listed first.
