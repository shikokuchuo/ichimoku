...
Takes the current numeric representation of the ichimoku cloud chart and compares it with the historical values contained within the ichimoku object.

```{r relative}
relative(cloud)[1:10, ]
```

The mean and standard deviation of each element is calculated, and the latest available datapoint is shown under 'current'. 'relative' is calculated as (current - mean) / sd and hence represents a centred and scaled 'relative' deviation.

Where this value is large (either positive or negative), this would indicate exceptional behaviour as opposed to being within normal ranges.

The elements are ordered by the absolute value of 'relative' so those with a higher relative deviation are always listed first.
