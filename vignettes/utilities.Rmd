...
- `...` not used.
- `noholidays` (optional) if set, bypasses the function logic and returns TRUE for all dates in 'x'.
...
Note that for ichimoku objects, a marginally faster, more specific version has been implemented as the S3 method `as.data.frame.ichimoku()`. Hence using this utility on an ichimoku object is not required.
...
- `keep.attr`  [default 'timestamp'] name of an attribute in 'new' to retain if it is present, provided as a character string.
...
If the attribute specified by 'keep.attr' is present in 'new', for example the 'timestamp' in pricing data returned by `oanda()`, this is retained. If the attribute is not found in 'new', the argument has no effect. All other non-required attributes are dropped.
