...
- Columns [numeric]:
  ...
  + `$cloudB` - cloud (min of senkouA, senkouB)
...
After completing an analysis, the resulting ichimoku objects can be stored directly to disk. The functions `ichimoku_write()` and `ichimoku_read()` provide direct tranlsation between the ichimoku objects in an R session with binary files stored in the Apache Arrow IPC file format.

`ichimoku_write()` takes 2 arguments, the ichimoku object to be written, and the file path/name of the object to be stored. A confirmation message will be printed to the console if the write process has been succesful.

`ichimoku_read()` takes a single argument, the file path/name of the file to be read. The return value of the function can be assigned to an object. Archive files restored using `ichimoku_read()` will re-create an ichimoku object (including an object containing a strategy) identical to the original.
...
- The ichimoku object specification has been updated in version 0.3.6. ichimoku objects created in versions prior to 0.3.6 will no longer work correctly with newer versions of the package. Please run `ichimoku()` on all old objects to re-create the object according to the new specification (all data is preserved)
...
The 'cpp11' package provides the C++ interface for the fast window functions, but the headers used are 'vendored' within ichimoku to ensure stability (as is permitted by 'cpp11') so there is no ongoing dependency.
