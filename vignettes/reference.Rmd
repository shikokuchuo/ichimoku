...
The returned output is designed to fit the screen; call `more()` to display up to 100 more rows. Note that above, print is called setting 'plot = FALSE' to only return the data to the console. The default print method for ichimoku objects also plots the cloud chart to the graphical device. 
...
Gao, C. (2021), *ichimoku: Visualization and Tools for Ichimoku Kinko Hyo Strategies*. R package version 1.3.1, <https://CRAN.R-project.org/package=ichimoku>.
