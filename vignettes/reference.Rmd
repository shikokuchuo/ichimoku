...
- `ticker` a ticker to identify the instrument, otherwise this is set to the name of the object supplied
...
- `keep.data` set to TRUE to retain additional data present in the object supplied as additional columns and/or attributes. Note: only columns of numeric type or type coercible to numeric are preserved, otherwise NAs are introduced.
...
To define other dates as non-trading days, additional custom holidays may be supplied via a 'holidays' argument to `ichimoku()` which is then passed on to the `tradingDays()` function.
...
If modifications have been made to the data of an ichimoku object, call `ichimoku()` on the modified object to re-create the object according to the correct specification.
