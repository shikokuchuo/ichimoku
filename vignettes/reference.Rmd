...
ichimoku has been designed to be fully pipeable and interfaces well with other R packages that return financial data.

As an example, the `getSymbols()` function from the 'quantmod' package currently assigns the data to an object but returns a text string containing the name of the object. ichimoku handles this automatically such that a chart can be created directly as per the below:
...
`ichimoku()` is implemented as a generic function that provides S3 methods for objects of class 'xts', 'data.frame' and 'matrix'. Hence, 'data.table' and 'tibble' work seamlessly with `ichimoku()`, as will most likely other classes that inherit the class 'data.frame'.
...
- To ensure compatibility with the ichimoku object specification v1, please upgrade to the latest package version and run `ichimoku()` on previously-created objects to re-create them according to the new specification (data is preserved)
