...
For reading data in the 'json' format, which is commonly returned by web APIs, the `fparse()` function from the 'RcppSimdJson' package is extremely performant. The function takes json strings as well as raw vectors directly. 'RcppSimdJson' is a dependency of ichimoku.
...
- RcppSimdJson - for processing data retrieved from external APIs

Optional:

- shiny, bslib - for interactive visualizations
- arrow - for archiving ichimoku objects as Apache Arrow files
