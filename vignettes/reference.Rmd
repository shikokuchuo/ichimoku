...
After completing an analysis, the resulting ichimoku objects can be stored directly to disk. The function `archive()` provides direct tranlsation between objects in an R session with binary files stored in the Apache Arrow IPC file format.

This function was created specifically for seamless archiving of ichimoku objects, and dataframes returned by `oanda()`.

#### Writing
`archive()` for write operations will take 2 arguments: the object to be written, and the file path/name of the object to be stored. A confirmation is printed to the console once the file has been written.
...
Data integrity validation is performed by the sha256 hashing algorithm from the 'openssl' package. If 'openssl' is not available, the archive function still works in all respects but without the additional validation. When an archive is read back, if the sha256 hash of the restored object matches the hash of the original that is stored in the archive, a 'data verified' message is printed to the console along with the matching sha256 hash. For reference, this hash value is of the raw vector of the character representation of the object including its attributes.

Note: ichimoku objects and dataframes retuned by `oanda()` can be expected to be restored exactly. For 'data.frame' objects in general, exact replication of attributes in restored objects vs. the original cannot be guaranteed in all cases due to current limitations in the R/Arrow API.
...
- Due to the change in ichimoku object specification, using `plot()` on an ichimoku object created in version 0.1.* will no longer work. The solution is to run `ichimoku()` on the old object to re-create the object according to the new specification (all data is preserved)
...
- openssl - for data integrity verification of archived objects
