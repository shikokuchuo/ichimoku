...
ichimoku works with data frames and tabular data. The `ichimoku()` function handles as input xts, data.frame, data.table, tibble, tsibble, matrix, and other data.frame compatible objects.
...
A section (rows 100 to 110) of the ichimoku object is shown above. Note that print is called setting 'plot = FALSE' to return just the data to the console. The default print method for ichimoku objects also plots the cloud chart to the graphical device.
...
As an ichimoku object inherits the ‘xts’ and ‘zoo’ classes, all existing methods for these classes should work seamlessly. For working with ichimoku objects, please see the section [re-exports](#re-exports).
...
This consolidates all relevant data into a single object. There is the additional benefit that it can now be plotted as a sub-plot beneath the main ichimoku cloud chart (see [custom sub-plots](#custom-sub-plots)).
...
If the relevant market trades continuously on a 24/7 basis, specify 'holidays = NULL'. This will bypass the logic of `tradingDays()` and return all dates for calculating the future cloud.
...
An optimised ichimoku S3 method for the base generic `as.data.frame()` is provided for converting an ichimoku object to a dataframe. Set the argument `keep.attrs = TRUE` to retain the attributes of the original ichimoku object.

S3 methods for the following 'zoo' generics are also provided:
...
As the extracted objects are simple base types, they can easily be modified as required.

The below shows how to faithfully re-create an ichimoku object from its components:

```{r bind}
index <- index(cloud)
coredata <- coredata(cloud)

cloud2 <- ichimoku(ichimoku(cbind(index, coredata), ticker = attr(cloud, "ticker")))
identical(cloud, cloud2)
```

Note specifically that ichimoku needs to be called twice - first to convert the matrix to an ichimoku object and then again to recalculate taking into account NA values correctly. The ticker name should also be set explicitly. 

A warning is issued about checking the validity of POSIX times, which may safely be ignored as the POSIXct values in 'index' were simply coerced to numeric during the `cbind()` operation and remain valid.
...
- gtable - for arranging grid graphics objects
