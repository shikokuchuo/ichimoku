...
- ticker - an identifier, which if set, will override the name of the object passed to ichimoku as the 'ticker'.
- periods - defaults to c(9, 26, 52), which are the correct parameters for traditional Ichimoku analysis.
- holidays - if not set, New Year's and Christmas day are defined as holidays by default. Functions such as 'holidayLONDON()' or 'holidayNYSE()' from the 'timeDate' package may be used to generate holidays for particular markets as appropriate. The only use of this parameter is to exclude the specified days when calculating the future cloud.
...
- rlang - for programming reasons arising from the use of ggplot2
