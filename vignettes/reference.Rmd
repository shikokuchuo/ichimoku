...
**Note on data retrieval:** it is optimal to retrieve data starting from further back than the period being analysed. This is as the various cloud lines are calculated using up to 52 values and then plotted ahead up to 26 periods, and hence there will be only be a full chart with all of the ichmoku cloud values from the 78th observation onwards (using default cloud periods). Run `ichimoku()` on the full dataset and then subset the data window when plotting to ensure that there is a full cloud for the entire chart.
...
- `keep.data` set to TRUE to preserve additional data present in the input object 'x' as additional columns and/or attributes. Note: only columns of numeric type or type coercible to numeric are preserved, otherwise NAs are introduced.
...
The input object may contain additional data not required for ichimoku analysis such as volume. To include such data in the ichimoku object specify `keep.data = TRUE` when calling `ichimoku()`.
...
This allows you to not have to deal with different objects for the same data. There is also the added benefit that it can now be plotted as a sub-plot beneath the main ichimoku cloud chart (see next section).
...
A plot with default parameters is shown above. The plot function contains built-in customisations to allow for most envisioned use cases. The example below demonstrates some of the arguments that can be supplied to customise the plot.

```{r plot2}
plot(cloud, window = "2020-05-01/2020-12-01", ticker = "一目均衡表", subtitle = "Sample Data Index", theme = "dark")
```
...
```{r plotr}
plot(cloud, window = "2020-05/", theme = "mono", type = "r")
```

An **S-type oscillator** is based on the formula for the stochastic indicator, as developed by George Lane and others. This is a 2-line indicator with fast and slow lines with look-back windows corresponding to the fast and medium cloud periods respectively (default 9 and 26). The fast line is quicker to react to price changes and the slow line can either provide confirmation or signal divergence.
...
```{r plots}
plot(cloud, window = "2020-05/", theme = "solarized", type = "s")
```

#### Custom Sub-plots

If additional input data is preserved within the ichimoku object, for example volume data, this can also be plotted in the sub-plot area by specifying the 'custom' argument. Other calculated metrics such as custom indicators can also be plotted if they are added as an additional column to the ichimoku object.

The character string supplied to 'custom' is a regular expression pattern that is matched to the column name and hence does not need to match it exactly, nor is case considered.

```{r plotbar}
plot(kumo, window = "2020-05/", theme = "solarized", type = "bar", custom = "volume")
```

To view a custom sub-plot, type must be specified as either 'bar' or 'line', otherwise the type setting will take precedence. On a bar chart, the bar colours correspond to the candle direction.

```{r plotline}
plot(kumo, window = "2020-05/", type = "line", custom = "volume")
```

#### Further Options

Internally, ichimoku's `plot()` method is a wrapper for `autoplot()` and `extraplot()`. As `plot()` returns the original ichimoku object (invisibly), to access the actual plot objects produced you must call those functions directly. `autoplot()` returns a ggplot object with classes 'gg' and 'ggplot' and `extraplot()` returns a grid graphics object with classes 'gtable', 'gTree', 'grob' and 'gDesc'.
...
- The first ichimoku object specification was released in version 0.3.51 - ichimoku objects created in versions prior to 0.3.51 will no longer work correctly with newer versions of the package. Upgrade to the latest package version and run `ichimoku()` on previously-created objects to re-create them according to the new specification (data is preserved)
...
- gridExtra - for high-level manipulation of graphics objects
...
Gao, C. (2021), *ichimoku: Visualization and Tools for Ichimoku Kinko Hyo Strategies*. R package version 1.1.5, <https://CRAN.R-project.org/package=ichimoku>.
