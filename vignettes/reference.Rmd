...
As an ichimoku object inherits the 'xts' and 'zoo' classes, all the existing methods for these classes should work seamlessly. For convenience, the following functions used by ichimoku are re-exported:

- `index()` *from 'zoo'* - to extract the index of an ichimoku object
- `coredata()` *from 'zoo'* - to extract the columns of an ichimoku object as a numeric matrix (without the date index)
- `xts()` *from 'xts'* - to re-create an 'xts' object from data and a date-time index, use `xts(data, index)`. Once created, call `ichimoku()` to convert into an ichimoku object.
...
- The ichimoku object specification has been updated in version 0.3.51. ichimoku objects created in versions prior to 0.3.51 will no longer work correctly with newer versions of the package. Please run `ichimoku()` on all old objects to re-create the object according to the new specification (all data is preserved)
- The following functions are re-exported to allow manipulation of ichimoku objects, from the 'zoo' package: `index()`, `coredata()`, from the 'xts' package: `xts()`
...
- cpp11 - provides the C++ headers for fast window functions used by ichimoku (required when compiling package from source)
